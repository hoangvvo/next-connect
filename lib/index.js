...
module.exports = ({
  ...
  connect.apply = function apply(req, res, next) {
  ...
  connect.handle = (req,res,next)=> connect(req,res,next);
  
  function connect(req, res, done = (err)=> {
    if (res.writableEnded === false && res.finished === false) {
      if (err) onError(err, req, res)
      else setImmediate(onNoMatch, req, res)
    }
  }) {
    let i = 0;
    const { handlers } = connect.find(req.method, req.url);
